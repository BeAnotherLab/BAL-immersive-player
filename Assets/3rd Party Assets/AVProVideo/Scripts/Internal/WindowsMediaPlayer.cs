...
namespace RenderHeads.Media.AVProVideo
{
	public sealed class WindowsMediaPlayer : BaseMediaPlayer
	{
		...
		{
			SetOptions(forceDirectShowApi, audioDeviceOutputName);
		}

		public void SetOptions(bool forceDirectShowApi, string audioDeviceOutputName)
		{
			...
			if (!_canPlay)
			{
				if (!_hasMetaData)
				{
					if (Native.HasMetaData(_instance))
					{
						...
						Debug.Log("[AVProVideo] Using playback path: " + _playerDescription + " (" + _width + "x" + _height + "@" + GetVideoFrameRate().ToString("F2") + ")");
			...
			if (_hasVideo)
			{
				...
				{	
					if (ptr != System.IntPtr.Zero)
					{
						...
						{
							Debug.Log("[AVProVideo] Texture size changed: " + _width + " X " + _height);
							OnTextureSizeChanged();
		...
		private void OnTextureSizeChanged()
		{
			...
			{
				Debug.LogWarning("[AVProVideo] You're using the GPU decoder with linear color-space setting in Unity.  This can cause videos to become washed out due to our GPU decoder path not supporting sRGB textures.  This can be fixed easily by adding 'col.rgb = pow(col.rgb, 2.2);' to any shader rendering the video texture.");
			}
		}
